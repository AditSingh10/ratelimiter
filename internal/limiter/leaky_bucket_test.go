...
func TestLBAllow_BasicAllow(t *testing.T) {
	lb := NewLeakyBucket(1, time.Second)

	// Background is a no-op context for tests
	allowed, _, _, err := lb.Allow(context.Background(), "client1")
	if err != nil {
...
func LBTestAllow_AllowExhausted(t *testing.T) {
	// create a bucket with capacity 1, rate 1
	lb := NewLeakyBucket(1, 1)

	// Background is a no-op context for tests
	allowed1, _, _, err1 := lb.Allow(context.Background(), "client1")
	allowed2, _, _, err2 := lb.Allow(context.Background(), "client1")
...
// concurrency testing
func LBTestAllow_Concurrent(t *testing.T) {
	lb := NewLeakyBucket(10, 100)
